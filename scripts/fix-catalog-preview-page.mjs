...
const catalogDir = path.join(ROOT_DIR, "app", "catalog", "[slug]");
const catalogPagePath = path.join(catalogDir, "page.tsx");
const lookInsidePath = path.join(
  ROOT_DIR,
  "components",
  "books",
  "LookInsidePreview.tsx"
);
...
const pageContent = String.raw`import { notFound } from "next/navigation";
...
const backupPath = backupFile(catalogPagePath);
fs.writeFileSync(catalogPagePath, pageContent, "utf8");

console.log("");
console.log("Página de catálogo corregida.");
console.log(`Archivo actualizado: ${path.relative(ROOT_DIR, catalogPagePath)}`);

if (backupPath) {
  console.log(`Backup creado: ${path.relative(ROOT_DIR, backupPath)}`);
}

if (!fs.existsSync(lookInsidePath)) {
  console.log("");
  console.log("ADVERTENCIA:");
  console.log(
    "No encontré components/books/LookInsidePreview.tsx. Debes crear ese componente para que el botón Leer fragmento funcione."
  );
} else {
  console.log("Componente LookInsidePreview encontrado.");
}

console.log("");
console.log("Siguientes pasos:");
console.log("1) npm run debug:book-cover -- --slug la-arquitectura-de-la-disciplina");
console.log("2) npm run dev");
console.log("3) Abre /catalog/la-arquitectura-de-la-disciplina");
