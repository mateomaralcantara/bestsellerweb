...
async function deepMetrics(buffer: ArrayBuffer) {
  ...
  const items = Array.from((opf || "").matchAll(/<item\b([^>]*)\/?\s*>/gi)).map((m) => ({
    href: attr(m[1] || "", "href"),
    mediaType: attr(m[1] || "", "media-type").toLowerCase(),
  }));
  ...
  return {
    ...
    inlineStyleAttributes: count(html, /\bstyle=[\"'][^\"']+[\"']/gi),
    classAttributes: count(html, /\bclass=[\"'][^\"']+[\"']/gi),
    h1: count(html, /<h1\b/gi),
    h2: count(html, /<h2\b/gi),
    h3: count(html, /<h3\b/gi),
    roleHeading: count(html, /\brole=[\"']heading[\"']/gi),
    semanticChapter: count(html, /epub:type=[\"'][^\"']*chapter[^\"']*[\"']/gi),
    paragraphs: count(html, /<p\b/gi),
    divs: count(html, /<div\b/gi),
    spans: count(html, /<span\b/gi),
    cssTextAlignCenter: count(css, /text-align\s*:\s*center/gi),
    cssTextAlignLeft: count(css, /text-align\s*:\s*left/gi),
    cssTextAlignJustify: count(css, /text-align\s*:\s*justify/gi),
    cssTextIndent: count(css, /text-indent\s*:/gi),
    cssMarginAuto: count(css, /margin-(?:left|right)\s*:\s*auto/gi),
    cssFontFamily: count(css, /font-family\s*:/gi),
    cssFontSize: count(css, /font-size\s*:/gi),
    cssLineHeight: count(css, /line-height\s*:/gi),
    cssWidthPx: count(css, /(?:width|min-width|max-width)\s*:\s*\d+(?:\.\d+)?px/gi),
    cssHeightPx: count(css, /(?:height|min-height|max-height)\s*:\s*\d+(?:\.\d+)?px/gi),
    cssAbsolutePosition: count(css, /position\s*:\s*absolute/gi),
    cssOverflowHidden: count(css, /overflow(?:-x|-y)?\s*:\s*hidden/gi),
    cssImportant: count(css, /!important\b/gi),
    cssPageBreaks: count(css, /(?:page-break-(?:before|after|inside)|break-(?:before|after|inside))\s*:/gi),
    cssSelectorsChapterTitle: count(css, /(?:chapter|capitulo|title|titulo|heading)[^{]*\{/gi),
    inlineTextAlignCenter: count(html, /style=[\"'][^\"']*text-align\s*:\s*center[^\"']*[\"']/gi),
    inlineTextAlignLeft: count(html, /style=[\"'][^\"']*text-align\s*:\s*left[^\"']*[\"']/gi),
    inlineTextAlignJustify: count(html, /style=[\"'][^\"']*text-align\s*:\s*justify[^\"']*[\"']/gi),
  };
}
...
async function analyze(slug: string) {
  ...
  const { data: asset, error: assetError } = await supabaseAdmin
    .from("book_assets")
    .select("storage_bucket, storage_path, created_at")
    .eq("book_id", book.id)
    .eq("asset_type", "epub")
  ...
  if (assetError || !asset?.storage_bucket || !asset.storage_path) {
    throw new Error(`EPUB asset missing: ${slug}`);
  }
  ...
  const [preflight, structure] = await Promise.all([
    analyzeEpubBuffer(buffer),
    deepMetrics(buffer),
  ]);
